...
export function render(ctx, host, shell) {
  ...
  async function load() {
    try {
      ...
      paintTabs();
      paintBody();
      // Non-blocking: computeRscState() (lib/rsc.js) reads the helper's
      // cached health synchronously and optimistically; this just keeps that
      // cache fresh and repaints once it resolves. Cached for 30s, so
